...
class Restro{   //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~class Restro~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
   ...
   public void food(){     //-----------------------------------------------------------------------void food ----------------------------------------------------------------------------//
	...
	fos = new FileOutputStream("F:\\Sumant\\Java\\1.Java Stuffs\\2135 Sumant Dusane\\restraunt Management System\\Bills\\BillHistory " + bill + ".txt");
	...
	for(i = 1; i <= n; i++)
	{
	  ...
	  try{
		...
		fos.write(("Bill :" + Integer.toString(bill)).getBytes());
		fos.write("\n".getBytes());
		fos.write(("DishName : " + name[i]).getBytes());
		fos.write("\n".getBytes());
		fos.write(("Quantity : " + Integer.toString(quantity[i])).getBytes());
		fos.write("\n".getBytes());
		fos.write(("Price : " + Integer.toString(price[i])).getBytes());
		fos.write("\n".getBytes());
		fos.write(("Amount : " + Integer.toString(amount[i])).getBytes());
		fos.write("\n".getBytes());
		fos.write(("Total : " + Integer.toString(total)).getBytes());
		fos.write("\n".getBytes());
		fos.close();
   ...
   public void invoice(){   //------------------------------->>-----------------------------------void invoice------------------------------->>--------------------------------------------------//
	...
	for(i = 1; i <= n; i++){
		...
		System.out.println("\t"+ i+1+"\t\t"+name[i]+"\t\t"+quantity[i]+"\t\t\t"+price[i]+"\t\t"+amount[i]); 		
...
public void bill(){   //------------------------------->>x<<-----------------------------------void bill------------------------------->>x<<--------------------------------------------------//
	...
	try{
	    ...
	    System.out.println("Enter the Bill Number:");
	    bill = s.nextInt();
	    fis = new FileInputStream("F:\\Sumant\\Java\\1.Java Stuffs\\2135 Sumant Dusane\\restraunt Management System\\Bills\\BillHistory " + bill + ".txt");
	    int size = fis.available();

	    for(i = 0; i < size; i++) {

		char chr = (char)fis.read();
    ...
    } // end of void bill   //------------------------------->>x<<-----------------------------------void bill------------------------------->>x<<--------------------------------------------------//
